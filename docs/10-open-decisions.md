...
| Per-site agent send window | S26-07 evaluates `outside_hours` against the company-wide `GeneralSettings::$sendWindowStart` → `$sendWindowEnd` via `SiteClock` (site timezone for site-scoped bindings, company timezone for company-scoped). A per-site override of that window is unset. Site access hours (`facility.site_info`) are when renters can reach a unit — they are not office hours and must not be reused for this. |
