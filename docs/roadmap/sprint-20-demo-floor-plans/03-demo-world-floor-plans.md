...
**Floor assignment.** 120 units per site over 4 floors, 30 each (`ceil(count / 4)`):
...
$units = Unit::query()
    ...
    ->sortBy(fn (Unit $u) => [
        (int) Str::afterLast($u->unit_number, '-'),   // 01..10 — the suffix
        $u->unit_number,                              // stable tiebreak
    ])
    ->values();
```

Sorting by suffix first and chunking evenly puts suffixes 01–03 of *every* class on planta baja,
04–06 on planta 1, and so on. Each floor gets a mix of all 12 classes, which is what makes
...
data. Give `MAD-05`'s top floor two orphan shapes and three uncovered units, behind a named
...
**Performance budget.** 20 documents, ~30 shapes each. Generation is string building; sanitizing
...
- [ ] Union of `matched` across a site's four floors equals that site's 120 unit numbers, for the
      four intact sites.
- [ ] `MAD-05` Planta 3 reports exactly 2 orphan shapes and its site reports 3 uncovered units.
...
| `imperfect_map_reports_expected_buckets` | 2 orphans, 3 uncovered, on `MAD-05` only |
