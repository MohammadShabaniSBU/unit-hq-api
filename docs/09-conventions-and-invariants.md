...
26b. **`VoiceBridgeToken.secret` is a system-generated bearer credential, not a pasted-in operator secret.** Invariant 26 covers credentials the operator already holds (Stripe keys, comms provider tokens) and can therefore re-enter. `VoiceBridgeToken.secret` is generated by the API (`Str::random(40)`) — applying #26 literally would make the value unlearnable. Exception (V03-04): the raw secret is returned once, and only once, on `store` and `regenerateSecret`. Every other response omits it. Subsequent views never re-display it; rotation is the only recovery path.
