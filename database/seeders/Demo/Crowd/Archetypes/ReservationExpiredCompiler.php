...
/**
 * Accepted, then expired (status + released hold) so the expired board column has rows.
 */
final class ReservationExpiredCompiler
{
    /**
    ...
    public static function compile(string $handle, DemoRng $rng): array
    {
        ...
        $acceptDay = $enrol + $rng->int(1, 4);
        $expireDay = $acceptDay + $rng->int(4, 12);
        ...
        return [
            ...
            $expireDay => static function (DemoWorld $world) use ($handle): void {
                JourneySupport::expireReservation($world, $handle);
            },
        ];
    }
}
