...
use App\Support\Facility\FloorMapCompiler;
use App\Support\Facility\FloorMapScene;
...
class SiteMapController extends Controller
{
    ...
    public function store(Request $request, Site $site): JsonResponse
    {
        ...
        $validated = $request->validate([
            'floor_name' => [
                ...
                'string',
                'max:255',
                Rule::unique('site_maps', 'floor_name')->where('site_id', $site->id),
            ],
            'svg_map' => ['required_without:scene', 'prohibits:scene', 'string'],
            'scene' => ['required_without:svg_map', 'prohibits:svg_map', 'array'],
            'sort_order' => ['nullable', 'integer', 'min:0', 'max:65535'],
        ]);

        $resolved = $this->resolveSvgAndScene($validated, $validated['floor_name']);

        $siteMap = DB::transaction(function () use ($site, $validated, $resolved): SiteMap {
            $siteMap = $site->siteMaps()->create([
                'floor_name' => $validated['floor_name'],
                'svg_map' => $resolved['svg_map'],
        ...
        $siteMap->setAttribute('id_match', SiteMapIdMatcher::match($site, $resolved['svg_map']));
    ...
    public function update(Request $request, SiteMap $siteMap): JsonResponse
    {
        ...
        $validated = $request->validate([
            'floor_name' => [
                ...
                'string',
                'max:255',
                Rule::unique('site_maps', 'floor_name')
                    ->where('site_id', $siteMap->site_id)
                    ->ignore($siteMap->id),
            ],
            'svg_map' => ['sometimes', 'prohibits:scene', 'string'],
            'scene' => ['sometimes', 'prohibits:svg_map', 'array'],
            'sort_order' => ['sometimes', 'integer', 'min:0', 'max:65535'],
        ]);

        $hasSource = array_key_exists('scene', $validated) || array_key_exists('svg_map', $validated);
        $svgChanged = false;
        $payload = $validated;

        if ($hasSource) {
            $floorName = $validated['floor_name'] ?? $siteMap->floor_name;
            $resolved = $this->resolveSvgAndScene($validated, $floorName);
            $payload['svg_map'] = $resolved['svg_map'];
            $payload['scene'] = $resolved['scene'];
            $svgChanged = true;
        }

        DB::transaction(function () use ($siteMap, $payload, $svgChanged): void {
            $siteMap->update($payload);
     ...
     * @return array{svg_map: string, scene: array<string, mixed>|null}
     */
    private function resolveSvgAndScene(array $validated, string $floorName): array
    {
        if (array_key_exists('scene', $validated) && is_array($validated['scene'])) {
            $scene = FloorMapScene::normalize($validated['scene']);
            $compiled = FloorMapCompiler::render($floorName, $scene);

            return [
                'svg_map' => SvgSanitizer::sanitize($compiled),
                'scene' => $scene,
            ];
        }

        if (array_key_exists('svg_map', $validated) && is_string($validated['svg_map'])) {
            return [
                'svg_map' => SvgSanitizer::sanitize($validated['svg_map']),
                'scene' => null,
            ];
        }

        throw ValidationException::withMessages([
            'svg_map' => ['Provide either a scene or an SVG map.'],
        ]);
    }
}
