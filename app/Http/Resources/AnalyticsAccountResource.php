...
class AnalyticsAccountResource extends BaseResource
{
    /** @return array<string, mixed> */
    public function toArray(Request $request): array
    {
        $registry = app(AnalyticsProviderRegistry::class);
        $providerValue = $this->provider->value;
        $fields = $registry->supports($providerValue)
            ? $registry->make($providerValue, [], $this->base_url ?? '')->credentialFields()
            : [];
